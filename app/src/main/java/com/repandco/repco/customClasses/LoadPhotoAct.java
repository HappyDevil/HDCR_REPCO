...
public class LoadPhotoAct extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if ((resultCode == RESULT_OK) && (requestCode != REQUEST_HEADER) && (requestCode != REQUEST_PHOTO) && (requestCode != REQUEST_VIDEO)) {
            context = this;
            {
                ...
                if (data != null) {
                    ...
                    if (requestCode == LOAD_POST_PHOTO) {
                        if (imageViewLoader.getImageView() != null)
                            imageViewLoader.getImageView().setImageURI(uri);
                    }
...
//                                    imageViewLoader.setLoadPhotoUrl();
                                        ...
                                        if (imageViewLoader.getProgressBar() != null)
                                            imageViewLoader.getProgressBar().setVisibility(View.INVISIBLE);
                                        if (imagesAdapter.create != null)
                                            imagesAdapter.create.setActivated(true);
                                    } else {
                            ...
                            .addOnProgressListener(new OnProgressListener<UploadTask.TaskSnapshot>() {
                                ...
                                public void onProgress(UploadTask.TaskSnapshot taskSnapshot) {
                                    imagesAdapter.create.setActivated(false);
                                    double progress = (100.0 * taskSnapshot.getBytesTransferred()) / taskSnapshot.getTotalByteCount();
                                    if (imageViewLoader.getProgressBar() != null)
                                        imageViewLoader.getProgressBar().setProgress((int) progress);
                                }
                            });
                }
            }
